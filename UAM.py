...
parser.add_argument('--experiment', type=str, default='HNSW_CAM_tiny_imagenet', help='location for saving trained models')
...
def main():
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        train(train_loader, model, optimizer, scheduler, epoch, log, index, num_classes=num_classes)
...
def train(train_loader, model, optimizer, scheduler, epoch, log, index, num_classes):
    losses = AverageMeter()
    losses.reset()
    ...
    for i, (images) in enumerate(train_loader):  # get HNSW index and feat_map
        d = images.size()
        ...
        if i == 0:
            num = d[0] * 2
        # train_map = feat_map.reshape(N, Cf, -1).max(2)
        train_map = np.mean(feat_map, axis=(2, 3))[:]
        for ind in range(d[0]*2):
            feature_map.append(train_map[ind, :])
            all_feature_map.append(feat_map[ind, :])
            inputs.append(images[ind, :])
            index.addDataPoint(i * num + ind, train_map[ind, :])

    index.createIndex({'post': 8}, print_progress=True)
    ...
    for j, (images) in enumerate(train_loader):
        ...
        weights = np.zeros((N, Cf), dtype=np.float32)
        ...
        T = 0
        for ind in range(N):
            t += np.mean(feature_map[j * num + ind], axis=0)
            T = max(T, np.max(feature_map[j * num + ind], axis=0))
            # print(np.mean(GradCAM_weights[i*num + ind], axis=0))
        t = t / N
        #print('t:', t)
        #print('T:', T)
    
        resul = [0 for _ in range(6)]  # use HNSW find neighbor to remove noise
        sum = [0 for _ in range(N)]
        for k in range(N):
            result, distances = index.knnQuery(feature_map[j*num + k], 5)
            resul[len(result)] += 1
            if len(result) < 1:
                for l in range(Cf):
                    if feature_map[j*num + k][l] > t:
                        weights[k, l] = 1.0
                        sum[k] += 1
            else:
                for l in range(Cf):
                    ...
                    if feature_map[j*num + k][l] <= t:
                        continue
                    if feature_map[j*num + k][l] > 0.9 * T:
                        weights[k, l] = 1.0
                        sum[k] += 1
                        continue
                    for re in result:
                        if feature_map[re][l] <= t:
                            flag = 0
                            break
                    if flag == 1:
                        weights[k, l] = 1.0
                        sum[k] += 1
        #print('image persist feature num:', sum)
        #print('result num:', resul)

        # erase the input under the guidance of UAM
        for a in range(N):
            ...
            # print('a:{}, x:{}, y:{}'.format(a, len(x), len(y)))
