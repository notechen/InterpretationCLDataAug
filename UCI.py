...
parser.add_argument('--experiment', type=str, default='counterfactual_cifar10_augmentation', help='location for saving trained models')
parser.add_argument('--data', type=str, default='dataset', help='location of the data')
parser.add_argument('--dataset', type=str, default='cifar10', help='which dataset to be used, (cifar10 or cifar100 or STL_10 or tiny_imagenet)')
...
def main():
    ...
    tfs_train = transforms.Compose([
        transforms.RandomResizedCrop(pic_size, scale=(1.0 - 0.5 * strength, 1.0), interpolation=3),  #STL_10 96*96    tiny_imagenet64*64
        transforms.RandomHorizontalFlip(),
        rnd_color_jitter,
        rnd_gray,
        transforms.ToTensor(),
        #transforms.RandomErasing(p=1, scale=(0.02, 0.33), ratio=(0.3, 3.3), value=1, inplace=True)
        #transforms.GaussianBlur(7, sigma=(0.5, 2.0))
    ])
...
def train(train_loader, model, optimizer, scheduler, epoch, log, num_classes):
    ...
    for i, (inputs) in enumerate(train_loader):
        ...
        model.zero_grad()
        inputs_adv = counterfactual_contrastive(model, inputs, iters=args.pgd_iter, singleImg=False)
        #features_adv = model.train()(inputs_adv, 'normal')
        CAM = torch.sum(torch.abs(inputs_adv - inputs), dim=1).cpu().numpy()

        # erase the input under the guidance of UCI
        for j in range(d[0]*2):
            # CIFAR-10/100 sigma=9, STL-10 and Tiny ImageNet sigma=18
            CAM[j] = gaussian_filter(CAM[j],sigma=9)
...
def counterfactual_contrastive(model, inputs, eps=8. / 255., alpha=2. / 255., iters=10, singleImg=False, feature_gene=None, sameBN=False):
    ...
    for i in range(iters):
        ...
        if torch.pow(torch.sum(one_hot * F.softmax(features, dim=1).cpu() - 1), 2) > b:
            a += 0.2
        loss = a*torch.pow(torch.sum(one_hot * F.softmax(features, dim=1).cpu() - 1), 2).cuda() + torch.sum(torch.pow(delta, 2)).cuda()
